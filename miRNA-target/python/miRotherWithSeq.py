...
headerList = ['chromosome', 'start', 'end', 'bindID','merClass',
    'strand', 'miRNAid', 'miRNAname', 'miRseq', 'align', 'targetSeq', 'tdmdScore',
    'clipExpNum', 'clipIDnum', 'RBP', 'RBPclipNum', 'RBPclipID', 'degraExpNum', 'degraIDNum', 'degraID']
...
with open(args.output, 'w') as out:
    ...
    for i in range(len(clipList)):
        ...
        degraExpNum = degradomeRow[11]
        degraIDNum = degradomeRow[12]
        degradID = degradomeRow[13]
        row.extend([str(tdmdScore), clipExpNum, clipIDnum, RBPcat, RBPClipNumCat,
            RBPclipIDcat, degraExpNum, degraIDNum, degradID])
        out.write('\t'.join(row) + '\n')
